...
exports.execute = (client, message, [args, ...words]) => {
    const regex = /[!*();,:@&=+$.\/?%#[\]]/g;
    ...
    if (args == 'list') {
        ...
        let embed1 = new Discord.MessageEmbed()
            .setColor("RANDOM")
            .setTitle('List of languages birb can translate:')
            .setDescription(listOfDLangs)
            .addField('How to use: ', '`b!translate lang1-lang2 stuff to translate`')
            .setTimestamp();
        ...
        if (args[2] !== "-")
            return message.channel.send('Please correctly format the command, like so\n\n`b!translate en-es "word or sentence to translate". Here, en is the language you are using and es is the language you want to convert it to.`');
        if (!langs.hasOwnProperty(targetLang))
            return message.channel.send(`Target Language \`${targetLang}\` doesn't exist.\n(if you believe this is, report it in our server!`);
        if (words == "")
