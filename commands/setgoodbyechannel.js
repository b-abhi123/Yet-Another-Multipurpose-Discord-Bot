...
let chans = JSON.parse(fs.readFileSync("./goodbyechannels.json", "utf8"))
    if (!chans[message.guild.id]){
      chans[message.guild.id] = {
        status: 'Aw **${member.user.discriminator}**, left ${message.guild.name} :('
      }
    }
...
if(!settingchannel) return message.channel.send('Mention a channel that I have access to!');
    ...
    chans[message.guild.id] = {
      status: settingchannel.id
       ...
       fs.writeFile("./goodbyechannels.json", JSON.stringify(chans), (err) => {
        if (err) console.log(err)
      })

    message.channel.send(`Successfully made ${settingchannel.id} as your new goodbye message channel!`)
...
exports.help = {
    ...
    aliases: ["setbyechannel"],
    usage: "setgoodbyechannel"
}